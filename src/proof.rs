...
impl<'a> Proof<'a> {
    ...
    // Whether this node is reachable from its predecessors by a basic proof.
    // This may involve more than one basic proof step.
    ...
    // Remove nodes that can be generated by only basic operations.
    fn remove_basic(&mut self) {
        for node_id in 0..self.nodes.len() as NodeId {
            if self.is_basic(node_id) {
